  ...
  self.create = function (w3cManifestInfo, rootDir, options, callback) {
    ...
    fileTools.mkdirp(platformDir)
      ...
      .then(function() {
        ...
        var iconPathName = url.parse(iconSrc).pathname || iconSrc;
        var iconFullPath = path.join(imagesDir, iconPathName);

        return Q.Promise(function(resolve, reject) {
          // see spec at https://en.wikipedia.org/wiki/Apple_Icon_Image_format
          var icnsPath = path.join(imagesDir, "app.icns");
          var icnsStream = fs.createWriteStream(icnsPath);
          var imageSize = fs.statSync(iconFullPath).size;
          
          var headerBuffer = new Buffer(8);
          headerBuffer.write('icns', 0, 'ascii');
          headerBuffer.writeUInt32BE(imageSize + (8 * 2), 4);
          
          icnsStream.write(headerBuffer, 'binary');

          var iconData = fs.readFileSync(iconFullPath);

          if (!iconData) {
            reject('Error while reading the icon data')
          };

          var iconBuffer = new Buffer(8);
          iconBuffer.write('icp5' + i, 0, 'ascii');
          iconBuffer.writeUInt32BE(8 + iconData.length, 4);
            
          icnsStream.write(iconBuffer, 'binary');
          icnsStream.write(iconData, 'binary');
